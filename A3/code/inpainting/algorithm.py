...
import cv2 as cv
import queue
import inpainting.copyutils as copyutils
from inpainting.patchdb import *
from inpainting.psi import *
from inpainting.copyutils import *
from inpainting.debug import *
...
class Inpainting:
    ...
    def writeImage(self, fileName, key):
        ...
        if image is not None:
            print(f"Debugging: {fileName}")
            fullFileName = f"{fileName}{key}.png"
            cv.imwrite(fullFileName, image)
            success = True
            msg = 'Image written successfully'
        #########################################
...
success, errorMessage = exampleBasedInpainting(self)
    ...
    def inspect_priority_queue(self):
        ...
        while not self._deltaOmega.empty():
            item = self._deltaOmega.get()
            temp_list.append(item)
        
        # Restore the items back to the queue
        for item in temp_list:
            self._deltaOmega.put(item)
    ...
    def computeBoundaries(self):
        ...
        boundaries, _ = cv.findContours(unfilled, cv.RETR_LIST, cv.CHAIN_APPROX_NONE)
